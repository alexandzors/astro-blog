...
featured: false
cover:
  alt: Image depicting the ntfy logo.
  image: /public/covers/2023/selfhosting-ntfy_banner.webp
draft: false
---
...
<div style="display: flex; justify-content: center;"><a href="#what-is-ntfysh"> > Skip to the goods < </a></div>

<<<<<<< Updated upstream:src/content/docs/posts/2023/09/12/selfhosting-ntfy.mdx
<p style="text-align: center;">So in my <a href="https://blog.alexsguardian.net/posts/2023/07/31/update">previous blog update post</a> I mentioned a few projects I was working on for posts on this blog. Well considering this post wasn't on the list it's time to toss the plan! Or did I really have a plan to begin with? :no_mouth:</p>
=======
<p style="text-align: center;">So in my <a href="https://blog.alexsguardian.net/posts_images/2023/07/31/update/">previous blog update post</a> I mentioned a few projects I was working on for posts on this blog. Well considering this post wasn't on the list it's time to toss the plan! Or did I really have a plan to begin with? :no_mouth:</p>
>>>>>>> Stashed changes:src/content/docs/blog/2023/09/12/selfhosting-ntfy.mdx

<center>![Plan Meme](https://media.makeameme.org/created/heres-the-plan-745ca82725.jpg)</center>

Currently, I have a few homelab notifications being sent to a "private" [Discord](https://discord.com) server. Evident by [Logging Fail2Ban to Discord](https://blog.alexsguardian.net/posts/2022/08/09/Fail2Ban+Discord/), [SSH Login Logging to Discord via Webhooks](https://blog.alexsguardian.net/posts/2022/07/02/LoggingSSHtoDiscord/), [GitHub Actions - Automating your Docker Builds with Diun](https://blog.alexsguardian.net/posts/2022/06/20/AutoGHActionsDiun/), a few others I was working on, etc. Now, I was going to keep going this route as it does make it extremely convenient since I already have the Discord app on most of my devices. However, I was talking to a [good friend of mine](https://github.com/StefanMarAntonsson), and he brought up a good point. 
...
![Notify Examples](/src/assets/posts_images/2023/09/12/ntfy_app_examples.webp)
_Examples of some notifications I am piping into my ntfy instance. Picture is the mobile app view_
...
- **A host to run ntfy on**
  - I will be using a host that is running [Docker](https://docker.com)[^1][^2], on Ubuntu, as it makes it easy to spin up resources without bloating the host with tons of dependencies.
...
Now add Docker's apt repository to your source list.
...
Then run apt update to update the sources again.
...
- **default-host** - This sets the default host for subscribing to notification topics. Since this is self-hosted your notify domain would go here.
- **base-url** - The URL in which you will access / communicate with ntfy. Usually the same as above ^.
- **listen-http** - The port in which the HTTP web server listens on (default = 80). You can add an IP to bind to as well (listen-http: "127.0.0.1:80")
- **cache-file** - Location of the message cache file.
- **behind-proxy** - If ntfy is deployed behind a proxy like Caddy, nginx, etc., then this needs to be set to `true`.
- **attachment-cache-dir** - Cache location for message attachments.
- **keepalive-interval** - The keepalive interval message interval in seconds.
- **auth-file** - Location of the `user.db` authentication file.
- **auth-default-access** - Setting this to "deny-all" will force all topics to be private by default.
- **web-root** - Location of the root directory for ntfy. Setting this to "disable" turns off the web client.

*You can see more configuration options in the [ntfy docs](https://docs.ntfy.sh/config/#config-options).*

For now, we will leave the web-root enabled to verify ntfy is running correctly. We will disable it later which is why it is commented out in the above config.

## Deploying ntfy

Change directory back to the parent directory for ntfy where the `docker-compose.yml` file is and deploy ntfy.
...
<Aside type="note">
  If you get an error that the port is already in use, you will have to edit the compose file and change `- 80:80` to something different. (example: `- 8080:80``). 8080 being the host side port.
</Aside>

## Configuring the Web Server

For my environment I use Caddy as my web server / reverse proxy. So this section will be covering how to configure Caddy to serve ntfy with an SSL certificate. If you don't have Caddy configured already, check out my quick start guide for [Caddy with Cloudflare/cloudflared](/posts_images/2023/12/04/caddyguide-part1/). Otherwise, you can check out the [ntfy docs for other web servers](https://docs.ntfy.sh/config/#nginxapache2caddy) / [skip this section](#securing-ntfy).
...
![ntfy.sh web app](/src/assets/posts_images/2023/09/12/ntfy_web.webp)
_Image from ntfy.sh's web app [https://ntfy.sh/app](https://ntfy.sh/app)_
...
![Disabled web client](/src/assets/posts_images/2023/09/12/ntfy_web_disabled.webp)
_Applying the above changes and refreshing your notify URL will update the page to this_
...
![Adding ntfy user](/src/assets/posts_images/2023/09/12/ntfy_user_add.webp)
_Example of creating a user using ntfy cli_
...
![Setting user access](/src/assets/posts_images/2023/09/12/ntfy_user_access.webp)
_Example of setting user access using ntfy cli_

```shell
ntfy access testuser ssh_logins wo
```
...
![Creating access token](/src/assets/posts_images/2023/09/12/ntfy_user_token.webp)
_Example of creating user access token using ntfy cli_
...
![Token example](/src/assets/posts_images/2023/09/12/ntfy_user_token_ex.webp)
_Example of the output of the `token add` command with the token underlined in red_
...
```bash title="sshd-login-ntfy EXAMPLE"
#!/bin/bash
...
![ntfy app users](/src/assets/posts_images/2023/09/12/ntfy_app_users.webp)
