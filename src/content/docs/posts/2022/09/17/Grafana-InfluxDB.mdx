...
excerpt: How to deploy InfluxDB, Telegraf, and how to connect them to Grafana.
date: 2022-09-17
...
draft: false
slug: posts/2022/09/17/Grafana-InfluxDB/
---
...
![InfluxDB Welcome to InfluxDB](/src/assets/posts_images/2022/09/17/grafana-influx-start.jpeg)
...
![InfluxDB Wizard Complete](/src/assets/posts_images/2022/09/17/grafana-influx_setup_complete.jpeg)
_Completed InfluxDB Wizard Page_

### Configuring InfluxDB for Telegraf

We need 3 pieces of information before we can set up Telegraf. Since we already have our organization name from earlier we can skip right to creating the data bucket for Telegraf. To do this click on the "Load Data" tab on the left-hand side of the InfluxDB web UI. Then click on "Buckets" and follow the steps below.
<Steps>
  <ol>
    <li>
      Click "+ Create Bucket" on the right-hand side
      <ul>
        <li>Name it Telegraf</li>
        <li>(Optional) Set up the Delete Data time period</li>
      </ul>
    </li>
    <li>Click Create</li>
  </ol>
...
<Steps>
  <ol>
    ...
    <li>
      Select your Telegraf bucket in the "Bucket" dropdown
    </li>
    <li>
      Select Docker
    </li>
    ...
    <li>
      Name the config
      <ul>
        <li>Click on "docker" under Plugins.</li>
        <li>Enter `unix:///var/run/docker.sock` for the docker endpoint</li>
        <li>Click Done</li>
      </ul>
    </li>
    <li>Click Create and Verify</li>
  </ol>
</Steps>
...
```dotenv title=".env Example" showLineNumbers=false
...
![InfluxDB Data Explorer](/src/assets/posts_images/2022/09/17/grafana-influxdb_dataxp.jpg)
...
<Steps>
  <ol>
    <li>
      Click on "+ Generate API Token" and select "Read/Write API Token"
    </li>
    <li>
      Give the API token a description (eg. Grafana)
    </li>
    <li>
      Select "All Buckets" under READ tab
    </li>
    <li>
      Select "Scoped" under WRITE tab but do not select any buckets from the list
    </li>
    <li>
      Click save.
    </li>
    <li>Click on your API Token you just created in the list. Then copy down the API Token</li>
  </ol>
</Steps>
...
Once Grafana is running again, open a new tab in your browser and navigate to `http://yourmachineip:3000`. Once logged in, follow the steps below to set up InfluxDB as a data source.

<Steps>
  <ol>
    <li>
      Click Settings on lower left-hand side
    </li>
    <li>
      Click Data Sources in the menu pop up
      <Image src="/src/assets/posts_images/2022/07/22/grafana-add_prometheus_1.jpg" alt="Grafana Data Source" width="2508" height="1288" />
    </li>
    <li>
      Click "Add data source" button
      <Image src="/src/assets/posts_images/2022/09/17/grafana-add_influx_DS.jpg" alt="Grafana Data Source 2" width="2508" height="1288" />
    </li>
    <li>
      Select InfluxDB from the list of sources
      <Image src="/src/assets/posts_images/2022/09/17/grafana-select_influx_DS.jpg" alt="Grafana Data Source 3" width="2508" height="1288" />
    </li>
    <li>
      Enter the following information into the Settings for the InfluxDB data source.
      <ul>
        <li>Name: **InfluxDB**</li>
        <li>Query Language: **Flux**</li>
        <li>URL: **http://influxdb:8086**</li>
        <li>Basic auth: *disabled*</li>
        <li>Organization: **lab**</li>
        <li>Token: **Grafana's InfluxDB API Token**</li>
        <li>(optional) Default Bucket: **telegraf**</li>
      </ul>
      <Image src="/src/assets/posts_images/2022/09/17/grafana-add_influx.jpg" alt="Grafana Data Source 4" width="2508" height="1288" />
    </li>
    <li>
      Click "Save & Test"
    </li>
    <li>Click Back to return to the added sources' pane.</li>
  </ol>
</Steps>

With InfluxDB added we can now add a simple Dashboard to display Telegraf's data. Since there was no pre-made ones for InfluxDB Flux queries we will have to make one from scratch. Click on the Dashboards menu option on the left hand side menu. Then click on New Dashboard. On the new dashboard page click on "Add new panel". Next select InfluxDB from the Data source dropdown. Then copy the following query into the query field.
