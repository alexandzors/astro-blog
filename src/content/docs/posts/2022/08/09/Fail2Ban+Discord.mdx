...
featured: false
draft: false
slug: posts/2022/08/09/Fail2Ban+Discord
---
...
![Fail2Ban Service Status](/src/assets/posts_images/2022/08/09/discord-fail2ban-status.jpg)
_Output of the `systemctl status` command showing the status of the Fail2Ban service_
...
```ini title="discord_notifications.conf"
...
With everything configured we can now start up the fail2ban service and then check its status.
...
![Fail2Ban Service Status](/src/assets/posts_images/2022/08/09/discord-fail2ban-status2.jpg)
_Output of the `systemctl status` command showing the running status of the Fail2Ban service_
...
![Fail2Ban Jail Started](/src/assets/posts_images/2022/08/09/discord-fail2ban-started.jpg)
_Discord channel message stating that the [sshd] jail has started_
...
![Fail2Ban Jail Restart](/src/assets/posts_images/2022/08/09/discord-fail2ban-restarted.jpg)
_Discord channel messages stating that the [sshd] jail has stopped and started_

<Aside type="danger" title="PROCEED WITH CAUTION!">
...
![Discord Fail2Ban Ban/Unban Messages](/src/assets/posts_images/2022/08/09/discord-fail2ban-ban.jpg)
_Discord channel messages showing ban and unban of an IP_
...
```ini title="discord_notifications.conf" showLineNumbers=false
-d '{"username":"Fail2Ban", "content":":bell: <hostname> - **[<name>]** **BANNED** IP: `<ip>` for <bantime> hours after **<failures>** failure(s). Here is some info about the IP: https://db-ip.com/<ip>. Unban by running: `fail2ban-client unban <ip>`"}'
```
