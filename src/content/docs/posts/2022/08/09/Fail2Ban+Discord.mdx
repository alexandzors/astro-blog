...
![Fail2Ban Service Status](/src/assets/series/discord-logging/discord-fail2ban-status.jpg){: .shadow }
...
hostname = <hostname>
```
{: file="discord_notifications.conf" }
...
![Fail2Ban Service Status](/src/assets/series/discord-logging/discord-fail2ban-status2.jpg){: .shadow }
...
![Fail2Ban Jail Started](/src/assets/series/discord-logging/discord-fail2ban-started.jpg){: .shadow }
_Discord channel message stating that the [sshd] jail has started_

Now to test Fail2Ban (without another host/network) we need to edit our `Jail.local`{: .filepath } config file again and remove our IP address from the `ignoreip` list as well as update the ban time so we don't have to wait long.
...
![Fail2Ban Jail Restart](/src/assets/series/discord-logging/discord-fail2ban-restarted.jpg){: .shadow }
...
![Discord Fail2Ban Ban/Unban Messages](/src/assets/series/discord-logging/discord-fail2ban-ban.jpg)
